...
input_col1 = widgets.VBox([
    ...
    widgets.FloatText(value=5.0,  description='UDL w (kN/m):'),
...
input_col2 = widgets.VBox([
    ...
    widgets.FloatText(value=1600., description='Tensile σ_tu (N/mm²):')
])
...
def train_ml_model():
    ...
    try:
        ...
        print(f"ℹ️  Built-in rows: {len(df_builtin)}")
        ...
        if os.path.exists('feedback_log.csv'):
            ...
            if list(df_fb_raw.columns) == expected_cols:
                ...
                if feedback_data_count:
                    print(f"ℹ️  Feedback rows: {feedback_data_count}")
            else:
...
def calculate_and_plot(_unused_event):
    ...
    with output_area:
        ...
        LIMITS = {
            "Span L (m)":        (1, 2_000),
            "UDL w (kN/m)":      (0, 200),
            "Number of Cables":  (1, 50),
            "Cable Spacing (m)": (0, 20),
            "Cable Diameter (mm)": (1, 200),
            "Tensile Strength σ_tu (MPa)": (100, 5_000),
        }
        violations = []
        if not (LIMITS["Span L (m)"][0] <= L <= LIMITS["Span L (m)"][1]):
            violations.append("Span L")
        if not (LIMITS["UDL w (kN/m)"][0] <= w <= LIMITS["UDL w (kN/m)"][1]):
            violations.append("UDL w")
        if not (LIMITS["Number of Cables"][0] <= n <= LIMITS["Number of Cables"][1]):
            violations.append("n")
        if not (LIMITS["Cable Spacing (m)"][0] <= spacing <= LIMITS["Cable Spacing (m)"][1]):
            violations.append("spacing")
        if not (LIMITS["Cable Diameter (mm)"][0] <= dia_mm <= LIMITS["Cable Diameter (mm)"][1]):
            violations.append("diameter")
        if not (LIMITS["Tensile Strength σ_tu (MPa)"][0] <= strength <= LIMITS["Tensile Strength σ_tu (MPa)"][1]):
            violations.append("σ_tu")

        if violations:
            print(
                "🚧 Input out of realistic range: "
                + ", ".join(violations)
                + ". ⚠️ Please keep values within ordinary bridge-design limits.(I know you are testing limits 😄!)"
            )
            return
        ...
        for value, label in [
            (sag_eng, "sag"),
            (H_kN,    "horizontal tension"),
            (σ_actual, "σ_actual"),
        ]:
            if (not math.isfinite(value)) or abs(value) > 1e9:
                print(
                    f"😵 Calculation blew up – {label} became {value}. "
                    "Check the inputs for realistic numbers."
                )
                return
     ...
     # ---- Printout -----------------------------------------------------
        print("\n--- RESULTS ---")
        print(f"L = {L} m | w = {w} kN/m | n = {n} | d = {dia_mm} mm "
              f"| σ_tu = {strength} MPa | s = {spacing} m | ϕ = {design_factor}")
        print(f"Cable area (single)     : {area_mm2:8.2f} mm²")
        print(f"Horizontal tension H    : {H_kN:8.2f} kN")
        print(f"Vertical reaction V     : {V_kN:8.2f} kN")
        print("-" * 32)
        print(f"🧮 Math sag f_eng       : {sag_eng:8.3f} m "
              f"({sag_ratio:.2f} % of span)")
        print(f"Approx. tension at supp : {T_kN:8.2f} kN")
        print(f"Actual stress σ         : {σ_actual:8.2f} MPa")
        print(f"Utilization             : {utilization:6.1f} % of σ_tu")
        print("⚠️  σ exceeds σ_tu!" if σ_actual > strength
              else "✅  σ within limit.")

        if sag_ml is not None:
            print("-" * 32)
            print(f"🤖 AI sag f_ml          : {sag_ml:8.3f} m "
        ...
        # ---- Tiny credit (HTML) ------------------------------------------
        display(HTML(f"<span style='font-size:8px;font-style:italic;'>{CREDIT_TEXT}</span>"))
